...
export default function App() {
  const [entries, setEntries] = useState(() => {
    try { const s = localStorage.getItem("cmp_entries"); return s ? JSON.parse(s) : []; } catch { return []; }
  });
  const [mechanics, setMechanics] = useState(() => {
    try { const s = localStorage.getItem("cmp_mechanics"); return s ? JSON.parse(s) : []; } catch { return []; }
  });
  ...
  useEffect(() => { try { localStorage.setItem("cmp_entries", JSON.stringify(entries)); } catch {} }, [entries]);
  useEffect(() => { try { localStorage.setItem("cmp_mechanics", JSON.stringify(mechanics)); } catch {} }, [mechanics]);
